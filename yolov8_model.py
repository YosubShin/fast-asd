...
# https://github.com/sieve-community/examples/blob/main/object_detection/yolov8/yolov8_model.py
                ...
                if frame_to_process is not None:
                    ...
                    for x, model_to_use in enumerate(models_to_use):
                        results = model_to_use.predict(
                            frame_to_process, conf=confidence_threshold, verbose=False)
                        results_dict = self.__process_results__(
                            results, model_to_use)
                        # Filter results based on confidence threshold
                        results_dict["boxes"] = [
                            box for box in results_dict["boxes"] if box["confidence"] > confidence_threshold]
                        combined_boxes.extend(results_dict["boxes"])

                    if max_num_boxes != -1 and len(combined_boxes) > max_num_boxes:
                        combined_boxes = sorted(
                            combined_boxes, key=lambda x: x["confidence"], reverse=True)[:max_num_boxes]
