...
pretrained_model_path = ".models/pretrain_TalkSet.model"
save_path = "save/"
...
def scene_detect(video_path, save=False, start_frame=0, end_frame=None):
    ...
    if start_frame != 0 or end_frame is not None:
        ...
        videoManager.set_duration(
            start_time=start_timecode, end_time=end_timecode)
...
def predict_faces(DET, start_frame=0):
    ...
    for fidx, fname in enumerate(flist):
        ...
        bboxes = DET.detect_faces(imageNumpy, conf_th=0.9, scales=[
                                  face_detection_scale])
        dets.append([])
        for bbox in bboxes:
            # dets has the frames info, bbox info, conf info
            dets[-1].append({'frame': fidx + start_frame,
...
def evaluate_network(s, files, device='cuda'):
            ...
            with torch.no_grad():
                for i in range(batchSize):
                    inputA = torch.FloatTensor(
                        audioFeature[i * duration * 100:(i+1) * duration * 100, :]).unsqueeze(0).to(device)
...
def visualization(tracks, scores):
    ...
    vOut = cv2.VideoWriter(os.path.join(
        pyaviPath, 'video_only.avi'), cv2.VideoWriter_fourcc(*'XVID'), 25, (fw, fh))
    colorDict = {0: 0, 1: 255}
...
def setup(device='cuda'):
    ...
    DET = initialize_detector(device)
...
def main(
    ...
    # Add debug print to confirm function entry
    print(f"TalkNet main function started with video: {video_path}")
    print(
        f"Parameters: start={start_seconds}, end={end_seconds}, device={device}")
    ...
    try:
        ...
        fps = float(fps_output.split(
            '/')[0]) / float(fps_output.split('/')[1]) if '/' in fps_output else float(fps_output)
            ...
            if ret and current_frame is not None:
                ...
                try:
                    ...
                    current_frame = cv2.cvtColor(
                        current_frame, cv2.COLOR_RGB2BGR)
                    count = 0
                    for p in frames_number_to_read:
                    ...
                    try:
                        ...
                        frames = [cv2.imread(os.path.join(
                            pyframesPath, '%06d.jpg' % i)) for i in range(int(duration * fps))]
        ...
        for scene_data in scene:
            new_scenes.append((FrameTimecode(timecode=int(scene_data[0].frame_num * (
                25 / fps)), fps=25), FrameTimecode(timecode=int(scene_data[1].frame_num * (25 / fps)), fps=25)))
        ...
        if face_boxes:
            ...
            for face_box in face_boxes:
                ...
                for i in range(1, len(frame_data), 5):
                    bbox = [float(frame_data[i]), float(frame_data[i+1]),
                            float(frame_data[i+2]), float(frame_data[i+3])]
                    conf = float(frame_data[i+4])
                    frame_faces.append(
                        {"frame": frame_number, "bbox": bbox, "conf": conf})
        ...
        for shot in new_scenes:
            ...
            if shot[1].frame_num - shot[0].frame_num >= min_track:
                ...
                allTracks.extend(track_shot(
                    faces[max(0, shot[0].frame_num - first_frame):shot[1].frame_num - first_frame]))
        print("Faces tracked in %.3f seconds." % (time.time() - t))

        print("Cropping faces...")
        for ii, track in enumerate(allTracks):
            if (end_seconds - start_seconds) * fps < STORE_FRAMES_IN_MEMORY_THRESHOLD:
                vidTracks.append(crop_video_from_frames(
                    track, os.path.join(pycropPath, '%05d' % ii), frames))
            else:
                vidTracks.append(crop_video(
                    track, os.path.join(pycropPath, '%05d' % ii)))
        ...
        files.sort()
        scores = evaluate_network(s, files, device)
        ...
        for tidx, track in enumerate(vidTracks):
            ...
            for fidx, frame in enumerate(track['track']['frame'].tolist()):
                # average smoothing
                s = score[max(fidx - 3, 0): min(fidx + 3, len(score) - 1)]
                s = numpy.mean(s)
                x1 = int(track['proc_track']['x'][fidx] -
                         track['proc_track']['s'][fidx])
                y1 = int(track['proc_track']['y'][fidx] -
                         track['proc_track']['s'][fidx])
                x2 = int(track['proc_track']['x'][fidx] +
                         track['proc_track']['s'][fidx])
                y2 = int(track['proc_track']['y'][fidx] +
                         track['proc_track']['s'][fidx])
                # print(frame, fidx, len(faces))
                faces[frame]['faces'].append({'track_id': tidx, 'raw_score': float(
                    s), 'x1': x1, 'y1': y1, 'x2': x2, 'y2': y2, 'speaking': bool(s >= 0)})
                if frame not in faces_by_frame:
                    faces_by_frame[frame] = []
                faces_by_frame[frame].append({'track_id': tidx, 'raw_score': float(
                    s), 'x1': x1, 'y1': y1, 'x2': x2, 'y2': y2, 'speaking': bool(s >= 0)})
                # print(frame, frame + int(25 * start_seconds))
                scene_data = get_scene_by_frame_number(
                    frame + int(25 * start_seconds))
                if scene_data is not None:
                    ...
                    faces_by_scene[scene_num][frame].append({'track_id': tidx, 'raw_score': float(
                        s), 'x1': x1, 'y1': y1, 'x2': x2, 'y2': y2, 'speaking': bool(s >= 0)})

        target_num_frames = (int(end_seconds * fps) -
                             int(start_seconds * fps)) + 1
        print(
        ...
        print(f"ERROR in TalkNet main function: {str(e)}")
        import traceback
        traceback.print_exc()
        raise
